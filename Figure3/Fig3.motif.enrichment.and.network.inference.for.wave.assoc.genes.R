...
pdf(paste0("step7-shockwaves/networks/arc.diagram.FOUR-STRUCTURES.pdf"),  height=8,  width=24)
for(STRUCT in c("AC", "SZ", "Liga", "MCLig")){
	#STRUCT<-"Liga"
...
######################
	...
	for(j in 1:max(DATNET$from)){
		...
		if(j==1)
			RANGE<-range(c(dat.gg$x, ARCS$x2))*1.1
		X2<-unique(ARCS$x2)
		...
		g1<-ggplot() + 
			...
			geom_arc(data=ARCS[,], aes(x0 = x00 , y0 = y00 , r = radius, start =3/2* pi  - start , end = pi/2 - end ,
				size=AUC*100, color=as.factor(ID)), alpha=0.5)+
